...
/**
   ...
   */
const Selectors={actions:{nextButton:'[data-action="quizaccess_tcquiz/waitforstudents-next_button"]'},regions:{numStudentsSpan:'[data-region="quizaccess_tcquiz/numberstudents_span"]'}},registerEventListeners=(sessionid,quizid,cmid,attemptid,POLLING_INTERVAL)=>{var updateNumStudentsEvent=setInterval((async()=>{await async function(sessionid,quizid,cmid,attemptid){var result=await fetch(M.cfg.wwwroot+"/mod/quiz/accessrule/tcquiz/quizdatateacher.php?requesttype=getnumberstudents&quizid="+quizid+"&sessionid="+sessionid+"&cmid="+cmid+"&attempt="+attemptid+"&currentquestion=0&sesskey="+M.cfg.sesskey,{method:"POST"}),responseXMLText=await result.text();await function(responseXMLText){var responseXML=(new DOMParser).parseFromString(responseXMLText,"text/html"),number_of_students=responseXML.getElementsByTagName("tcquiz").item(0).getElementsByTagName("numberstudents").item(0).textContent;document.querySelector(Selectors.regions.numStudentsSpan).innerHTML=number_of_students}(responseXMLText)}(sessionid,quizid,cmid,attemptid)}),POLLING_INTERVAL);document.querySelector(Selectors.actions.nextButton).addEventListener("click",(async e=>{e.preventDefault(),clearInterval(updateNumStudentsEvent),updateNumStudentsEvent=null;var result=await fetch(M.cfg.wwwroot+"/mod/quiz/accessrule/tcquiz/quizdatateacher.php?requesttype=getquestion&quizid="+quizid+"&cmid="+cmid+"&attempt="+attemptid+"&sessionid="+sessionid+"&rejoin=0&sesskey="+M.cfg.sesskey,{method:"POST"}),responseXMLText=await result.text();await function(responseXMLText){var responseXML=(new DOMParser).parseFromString(responseXMLText,"text/html"),next_url=responseXML.getElementsByTagName("tcquiz").item(0).getElementsByTagName("url").item(0).textContent;window.location.replace(next_url)}(responseXMLText)}),{once:!0})};_exports.init=(sessionid,quizid,cmid,attemptid,POLLING_INTERVAL)=>{registerEventListeners(sessionid,quizid,cmid,attemptid,POLLING_INTERVAL)}}));
